---
id: 1
title: architecture-sketch-textbook-system
stage: plan
date: 2025-12-12
surface: agent
...
command: /sp.plan
labels: ["architecture", "textbook", "docusaurus", "physical-ai", "humanoid-robotics"]
links:
  ...
  pr: null
files:
 - specs/2-docusaurus-textbook/architecture-sketch.md
tests:
 - specs/2-docusaurus-textbook/architecture-sketch.md validates architecture requirements
---
...
A high-level architectural outline of the textbook system, showing how content, agents, simulations, and deployment integrate:

Specification Layer (Source of Truth)

Module and chapter specifications
...
Simulation reproducibility standards

Authoring & Generation Layer (AI-Assisted)

Chapter Generator Subagent

Code Generation Subagent (ROS 2, Python, Gazebo, Isaac)

Diagram Agent (URDF models, SLAM flows, VLA pipelines)

Research Agent (technical verification)

QA Agent (consistency + correctness)

Rendering, Personalization & Localization Layer

Docusaurus for static content

GitHub Pages for deployment

Better-Auth for signup/signin

Content personalization (experience-based, hardware-aware)

Urdu translation per chapter

Beginner/Advanced content modes
...
"Answer from selssic, Gazebo Ignition, Isaac Sim
...
No broken links, images, or formatting

Urdu translation matches original meaning

Beginner/Advanced modes both render correctly

2. Code Testing

ROS nodes run successfully

Gazebo worlds load error-free

Isaac Sim scenes open correctly

Nav2 and VSLAM operate as expected

3. RAG Testing

Embeddings generated per chapter

"Selected text only" queries return correct responses

Stress test with 50 random questions per module

Validate Neon + Qdrant connectivity

4. UI/UX Testing

GitHub Pages loads all pages

Mobile compatibility

Interactive buttons (translate, personalize) work

5. Edge Case Testing

No-GPU learner workflow

Cloud-simulation-only workflow

Low-bandwidth browsing mode

Technical Details

Use research-concurrent approach (research and writing happen in parallel).

Follow APA citation style as defined in the Constitution.

Organize book development by phases:

Phases

Research

Gather sources, validate robotics concepts

Identify required tools, packages, simulations
...
Map learning goals to content

Define code + simulation examples
...
Integrate code, diagrams, simulations

Deploy to GitHub Pages

Integrate RAG backend

Run QA + simulation tests
...
- ✅ Impact: Created comprehensive architecture sketch covering all requested layers and components
- 🧪 Tests: Architecture sketch validates all architectural requirements from user input
- 📁 Files: Created specs/2-docusaurus-textbook/architecture-sketch.md with complete architecture
- 🔁 Next prompts: Ready for implementation planning based on this architecture
- 🧠 Reflection: Architecture addresses all major concerns including content, personalization, simulation, and deployment

## Evaluation notes (flywheel)

- Failure modes observed: None - all architectural requirements successfully addressed
- Graders run and results (PASS/FAIL): PASS - architecture covers all requested components
- Prompt variant (if applicable): Original user request fully implemented
- Next experiment (smallest change to try): Begin implementation of specific architectural components